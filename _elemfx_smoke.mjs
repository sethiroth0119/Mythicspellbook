...
/* ── the registry ─────────────────────────────────────────────────────────── */
ok(/const _ELEMENT_FX_FILE = 'assets\/vfx\/vfx-elements';/.test(SRC), 'one page backs every effect');
{
  ...
  const ixs = (blk.match(/ix: (\d+)/g) || []).map(s => +s.replace('ix: ', ''));
  ok(new Set(ixs).size === ixs.length, 'no two entries point at the same effect index');
