...
ok(/if \(candidates\.length === 0 && graveReactions\.length === 0 && tokenHolders\.length === 0\) return false;/.test(SRC),
  '…and the window now OPENS for a field card alone — before, a player holding a fully-charged counter card saw nothing at all');
