...
/* ── 1. the clinic ── */
ok(/const got = s\.input \? svcDraw\(s\.input, svcWantFor\(def, s, mult, omRes, dtMin\), def\) : 1;/.test(NC), 'the tick asks svcWantFor how much a service may draw');
{
...
ok(/src\/farm\/index\.js\?v=v121v118farm5/.test(SRC), 'the farm buster moved');
console.log(fails ? '\n' + fails + ' FAILED' : '\nALL PASS');
