...
ok(/^function _haulModelOf\(v\) \{/m.test(SRC) && /const def = \(typeof _ppRigDef === 'function'\) \? _ppRigDef\(v\) : null;\n\s*const m = def && def\.model;\n\s*if \(m && typeof m\.url === 'string' && m\.url\) return/.test(SRC), 'the class model (the semi for freight) comes first for the haul; a row upload only when the class has none');
