  ...
  '💥 Punish Arrivals is an effect type, and its damage is an authored number');
{
  ...
  ok(/'punishEntry'\]/.test(g) && /'exileIfNotFromHand'\]/.test(g) && /'auraSilence'\]/.test(g),
...
/* ── 2. 💥 punish arrivals, executed ──────────────────────────────────────── */
{
  ...
  const code = slice('_authoredEffectsOf') + slice('_unitDeclaresEffect') + slice('_boardDeclarers') + slice('_punishEntry')
    + '\nreturn { _punishEntry };';
