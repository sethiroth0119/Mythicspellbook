...
ok(/if \(typeof isCardReleased === 'function' && !isCardReleased\(id\)\) \{[\s\S]{0,300}return 0;/.test(SRC), 'grantCard — the one door a card enters a collection through — refuses an unreleased card');
ok(/if \(isForgeAdminOnly\('card', cid\)\) return false;\n\s*return \(typeof isCardReleased === 'function'\) \? isCardReleased\(cid\) : true;   \/\/ 🔒 v121v122/.test(SRC), 'every "pick a random card" pool drops it too');
...
/* ── 4. run the release maths for real ── */
{
  ...
  const api = new Function('g', 'with (g) { ' + block + ' return { cardReleaseAt, isCardReleased, packReleaseAt, isPackReleased, releaseWhenText, getCardSets }; }')(g);
